...
from .tof_var import ToFDepthVAR
...
def build_vae_var(
    ...
    # VQVAE args
    V=4096, Cvae=32, ch=160, share_quant_resi=4, in_channels=3,
    # VAR args
    ...
    # build models
    vae_local = VQVAE(
        vocab_size=V, z_channels=Cvae, ch=ch, test_mode=True, share_quant_resi=share_quant_resi,
        v_patch_nums=patch_nums, in_channels=in_channels
    ).to(device)
    var_wo_ddp = VAR(
        vae_local=vae_local,
